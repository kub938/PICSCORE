...
//   useEffect(() => {
//     // 이미 로그인 처리가 되어있지 않고, URL에 토큰이 있으면 임시 저장
//     if (accessToken && loginSuccess) {
//       // 실제 로그인은 약관 동의 후에 처리됨
//       localStorage.setItem("tempAccessToken", accessToken);
//     } else if (!accessToken) {
//       // 토큰 없이 직접 약관 페이지에 접근한 경우 로그인 페이지로 리다이렉트
//       navigate("/login");
//     }
//   }, [accessToken, loginSuccess, navigate]);
...
//       // 여기서 약관 동의 상태를 백엔드로 전송하는 API 호출
//       // 실제 구현 시 userApi.agreeToTerms(tempToken, { required: requiredTermsChecked, marketing: marketingTermsChecked })
//       // 현재는 백엔드 연동 없이 성공했다고 가정
...
//         <h1 className="text-2xl font-bold text-center mb-2">서비스 이용 약관</h1>
...
//             이 약관은 PicScore(이하 "회사"라 함)가 제공하는 사진 평가 및 공유 서비스(이하 "서비스"라 함)를 이용함에 있어 회사와 이용자의 권리, 의무 및 책임사항을 규정함을 목적으로 합니다.
//           </p>

//           <p className="mb-2">제 2 조 (정의)</p>
//           <p className="text-gray-700 mb-3">
//             1. "서비스"란 회사가 제공하는 모든 서비스를 의미합니다.<br />
//             2. "이용자"란 회사의 서비스에 접속하여 이 약관에 따라 회사가 제공하는 서비스를 받는 회원 및 비회원을 말합니다.<br />
...
//           <p className="text-gray-700 mb-3">
//             1. 이 약관은 서비스를 이용하고자 하는 모든 이용자에 대하여 그 효력이 발생합니다.<br />
//             2. 회사는 필요한 경우 약관을 변경할 수 있으며, 변경된 약관은 회사가 제공하는 서비스 화면에 게시하거나 기타의 방법으로 이용자에게 공지함으로써 그 효력이 발생합니다.
//           </p>
...
//             - 사진 분석 및 평가 서비스<br />
//             - 사진 공유 및 소셜 네트워킹 서비스<br />
//             - 각종 정보 제공 서비스<br />
//             2. 회사는 서비스의 내용을 변경할 수 있으며, 이 경우에는 변경된 서비스의 내용 및 제공일자를 명시하여 서비스 화면에 공지합니다.
//           </p>
//         </div>
...
//         <h2 className="text-lg font-semibold mb-2">마케팅 정보 수신 동의 (선택)</h2>
//         <div className="h-24 overflow-y-auto bg-gray-50 p-3 rounded mb-3 text-sm">
//           <p className="mb-2">마케팅 정보 수신 동의</p>
//           <p className="text-gray-700 mb-3">
//             PicScore는 이용자의 마케팅 정보 수신 동의를 받고 다양한 이벤트, 프로모션, 뉴스레터, 서비스 업데이트 등의 정보를 이메일, SMS, 앱 푸시 알림 등의 방법으로 제공할 수 있습니다.
//             이 동의는 선택사항이며, 동의하지 않더라도 기본 서비스 이용에는 제한이 없습니다.
//           </p>
//         </div>
//         <label className="flex items-center text-sm">
//           <input
